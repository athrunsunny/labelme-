...
image_dir = r'G:\hp_tracking_proj\three_identy_data\union_val\images'
label_dir = r'G:\hp_tracking_proj\three_identy_data\union_val\labels'
...
train_image_des_dir = r'G:\hp_tracking_proj\three_identy_data\wider_val\images'
if not os.path.exists(train_image_des_dir):
    os.makedirs(train_image_des_dir)

label_image_des_dir = r'G:\hp_tracking_proj\three_identy_data\wider_val\labels'
...
def create_last_label(image_name):
    ...
    for im_name in tqdm(image_name):
        new_name = name + (4-int(len(str(idx)))) * '0' + str(idx)
        image_n = f'{image_dir}/{im_name}.jpg'
        frame = cv2.imread(image_n)
        new_image_n = f'{train_image_des_dir}/{new_name}.jpg'

        out_file = open('%s/%s.txt' % (label_image_des_dir, new_name), 'w')
