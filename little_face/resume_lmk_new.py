...
def resume(single_face,des, path=ROOT_DIR, padsz=5):
    resume_path = os.path.join(des, 'resume')
    ...
    for file in tqdm(jsonfiles):
        ...
        for item in shapes:
            ...
            if single_face_json_path in singlefacefiles:
                ...
                if lmk_points[0][0][0] <= -1 and lmk_points[0][0][1] <= -1 and lmk_points[-1][0][0] <= -1 and lmk_points[-1][0][1] <= -1 and \
                    lmk_points[1][0][0] <= -1 and lmk_points[1][0][1] <= -1 and lmk_points[2][0][0] <= -1 and lmk_points[2][0][1] <= -1 and \
                    lmk_points[3][0][0] <= -1 and lmk_points[3][0][1] <= -1:
                    # 关键点本身为-1
                    lmk_points_np = np.array(lmk_points,dtype=np.float64).reshape([1, 10])[0]
                else:
                    lmk_points_np = np.array(lmk_points,dtype=np.float64).reshape([1, 10])[0] - np.float64(padsz)
...
if __name__ == '__main__':
    single_face_path = r'G:\hp_tracking_proj\single_face_process\widerface_head_body_pose_lmk\wait_resume\sz_clean\des_path'
    des_path = r'G:\hp_tracking_proj\single_face_process\widerface_head_body_pose_lmk\wait_resume\sz_clean'
    resume(single_face_path,des_path)
